...
const { DEX_ROUTERS, WRAPPED_NATIVE, STABLECOINS, NETWORK_CONFIGS } = require('./networkconfig');
...
// QuickSwap V2 Router ABI (Uniswap V2 compatible)
const QUICKSWAP_V2_ROUTER_ABI = [
...
class SwapService {
  ...
  // Check if DEX uses V2 protocol (like QuickSwap, PancakeSwap V2)
  isDexV2(dexType) {
    const v2Dexes = ['quickswap', 'pancakeswap', 'sushiswap', 'traderjoe', 'pangolin', 'camelot', 'velodrome', 'aerodrome'];
    return v2Dexes.includes(dexType);
  }
  ...
  async swapNativeForToken(tokenOut, amountIn, slippagePercent = 10, dexType = null) {
    try {
      ...
      // Check if using V2 protocol (QuickSwap, etc.)
      if (this.isDexV2(dex)) {
        console.log(`Using ${dex} V2 protocol for swap`);
        const router = new ethers.Contract(routerAddress, QUICKSWAP_V2_ROUTER_ABI, this.signer);
        
        // Path: Native -> Token
        const path = [wrappedNative, tokenOut];
        ...
        try {
          ...
          amountOutMin = expectedOut * BigInt(100 - slippagePercent) / BigInt(100);
          console.log(`Expected output: ${ethers.formatUnits(expectedOut, tokenDecimals)} ${tokenSymbol}`);
          console.log(`Minimum output (${slippagePercent}% slippage): ${ethers.formatUnits(amountOutMin, tokenDecimals)} ${tokenSymbol}`);
        } catch (e) {
          console.log('Could not get quote, using 0 as minimum (be cautious!)');
        }
        
        console.log(`Executing ${dex} V2 swap...`);
      ...
      // Use V3 (Uniswap V3) for non-V2 DEXes
      const isRouter02 = this.isSwapRouter02(routerAddress);
      console.log(`Using ${isRouter02 ? 'SwapRouter02' : 'SwapRouter'} for Uniswap V3 swap on ${dex}`);
      ...
      if (isRouter02) {
        ...
        console.log(`Executing Uniswap V3 swap via ${dex} (SwapRouter02)...`);
        ...
        console.log(`Executing Uniswap V3 swap via ${dex} (Legacy Router)...`);
  ...
  async swapTokenForNative(tokenIn, amountIn, slippagePercent = 10, dexType = null) {
    try {
      ...
      if (this.isDexV2(dex)) {
        ...
        // Path: Token -> Native
        const path = [tokenIn, wrappedNative];
        ...
        try {
          ...
          const expectedOut = amounts[amounts.length - 1];
          amountOutMin = expectedOut * BigInt(100 - slippagePercent) / BigInt(100);
          console.log(`Expected output: ${ethers.formatEther(expectedOut)} ${this.networkConfig.symbol}`);
          console.log(`Minimum output (${slippagePercent}% slippage): ${ethers.formatEther(amountOutMin)} ${this.networkConfig.symbol}`);
        } catch (e) {
          console.log('Could not get quote, using 0 as minimum (be cautious!)');
        }
      ...
      // Use V3 (Uniswap V3) for non-V2 DEXes
      const isRouter02 = this.isSwapRouter02(routerAddress);
      console.log(`Using ${isRouter02 ? 'SwapRouter02' : 'SwapRouter'} for Uniswap V3 swap`);
      ...
      if (isRouter02) {
        ...
        console.log(`Executing Uniswap V3 swap via ${dex} (SwapRouter02 with unwrap)...`);
    ...
    try {
      ...
      if (this.isDexV2(dex)) {
        ...
        try {
          ...
          const expectedOut = amounts[amounts.length - 1];
          amountOutMin = expectedOut * BigInt(100 - slippagePercent) / BigInt(100);
          console.log(`Expected output: ${ethers.formatUnits(expectedOut, decimalsOut)} ${symbolOut}`);
          console.log(`Minimum output (${slippagePercent}% slippage): ${ethers.formatUnits(amountOutMin, decimalsOut)} ${symbolOut}`);
        } catch (e) {
          console.log('Could not get quote, using 0 as minimum (be cautious!)');
        }
